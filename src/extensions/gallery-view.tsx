    ...
    >
      <div className="mb-4 flex items-center justify-between text-[11px] text-[#666]">
        ...
        <button
          ...
          className="text-[#4cc2ff] hover:text-[#7b9eff] font-medium"
        >
          Add Images
        </button>
        <input
      ...
      {images.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-[#333] rounded-lg bg-[#222]">
          ...
          <button
            onClick={() => fileRef.current?.click()}
            className="px-4 py-1.5 bg-[#333] hover:bg-[#444] text-[#e0e0e0] rounded-md text-[12px] font-medium transition-colors"
          >
            Browse Files
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {images.map((img, i) => (
            <div
              key={i}
              className="relative group aspect-square rounded-md overflow-hidden bg-[#222] ring-1 ring-[#333]"
            >
